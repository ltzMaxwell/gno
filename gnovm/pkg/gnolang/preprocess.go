...
func Preprocess(store Store, ctx BlockNode, n Node) Node {
	...
	nn := Transcribe(n, func(ns []Node, ftype TransField, index int, n Node, stage TransStage) (Node, TransCtrl) {
		...
		// debugPP.Printf("Preprocess %s, (%v) stage:%v\n", n.String(), reflect.TypeOf(n), stage)
			...
			case *SwitchClauseStmt:
				...
				if ss.IsTypeSwitch {
					...
					for i, cx := range n.Cases {
						cx = Preprocess(
							store, last, cx).(Expr)
						checkOrConvertType(store, last, &cx, tt, false, false) // #nosec G601
						n.Cases[i] = cx
					}
				}
		...
		case TRANS_LEAVE:
			...
			case *BinaryExpr:
				...
				if lic {
					if ric {
						...
						return cx, TRANS_CONTINUE
					} else if isUntyped(lcx.T) { // untyped -> not const
						// Left untyped const, Right not const ----------------
						if rnt, ok := rt.(*NativeType); ok { // untyped -> gno(native) TODO: check this case
						...
						if lnt, ok := lt.(*NativeType); ok {
							...
							if liu {
								if riu {
									...
									debugPP.Println("left untyped, right is typed")
									checkOperandWithOp(store, last, &n.Left, rt, n.Op, Binary)
									checkOrConvertType(store, last, &n.Left, rt, false, false)
								}
			...
			case *CallExpr:
				...
				case *TypeType:
					...
					case *UnaryExpr:
						debugPP.Printf("---callExpr, arg is unary expr, bx: %v \n", arg0)
						if isUntyped(at) {
							checkOrConvertType(store, last, &n.Args[0], ct, false, true)
						}
					default:
						debugPP.Printf("default: arg type is: %T \n", arg0)
					}
			...
			case *IndexExpr:
				...
				case MapKind:
					...
					checkOrConvertType(store, last, &n.Index, mt.Key, false, false)
				default:
					panic(fmt.Sprintf(
			...
			// TRANS_LEAVE -----------------------
			case *TypeAssertExpr:
				debugPP.Println("TypeAssertExpr---")
				if n.Type == nil {
					panic("should not happen")
				}
			...
			// TRANS_LEAVE -----------------------
			case *UnaryExpr:
				debugPP.Println("UnaryExpr---")
				xt := evalStaticTypeOf(store, last, n.X)
				debugPP.Printf("---unaryExpr---, op: %v, xt: %v \n", n.Op, xt)
				if xnt, ok := xt.(*NativeType); ok {
					debugPP.Println("nativeType")
					// get concrete native base type.
				...
				if cx, ok := n.X.(*ConstExpr); ok {
					debugPP.Println("not native, ConstExpr")
			...
			CLT_TYPE_SWITCH:
				...
				case *StructType:
					if n.IsKeyed() {
						for i := 0; i < len(n.Elts); i++ {
							...
							checkOrConvertType(store, last, &n.Elts[i].Value, ft, false, false)
						}
					} else {
						for i := 0; i < len(n.Elts); i++ {
				...
				case *ArrayType:
					for i := 0; i < len(n.Elts); i++ {
						...
						checkOrConvertType(store, last, &n.Elts[i].Key, IntType, false, true)
						checkOrConvertType(store, last, &n.Elts[i].Value, cclt.Elt, false, false)
					}
				case *SliceType:
					for i := 0; i < len(n.Elts); i++ {
						// bypass check as a special case
				...
				case *MapType:
					for i := 0; i < len(n.Elts); i++ {
						checkOrConvertType(store, last, &n.Elts[i].Key, cclt.Key, false, false)
						checkOrConvertType(store, last, &n.Elts[i].Value, cclt.Value, false, false)
					}
				case *NativeType:
			...
			// TRANS_LEAVE -----------------------
			case *KeyValueExpr:
				debugPP.Println("KeyValueExpr---")
				// NOTE: For simplicity we just
				// use the *CompositeLitExpr.

			// TRANS_LEAVE -----------------------
			case *SelectorExpr:
				debugPP.Println("SelectorExpr---")
				xt := evalStaticTypeOf(store, last, n.X)
				...
				if len(n.Results) != len(ft.Results) {
					...
					for i := range n.Results {
						...
						if isGeneric(rt) {
							...
							checkOrConvertType(store, last, &n.Results[i], rt, false, false)
						}
					}
				}
			...
			case *SendStmt:
				...
				checkOrConvertType(store, last, &n.Value, nil, false, false)
				...
				if numNames > 1 && len(n.Values) == 1 {
					...
					if n.Type != nil {
						...
						for i := range n.Values {
							checkOrConvertType(store, last, &n.Values[i], nt, false, false)
						}
					} else if n.Const {
...
func evalStaticType(store Store, last BlockNode, x Expr) Type {
	debugPP.Printf("---evalStaticType of: %v \n", x)
...
func evalStaticTypeOf(store Store, last BlockNode, x Expr) Type {
	debugPP.Printf("---evalStaticTypeOf: x : %v \n", x)
	...
	if cx, ok := (*x).(*ConstExpr); ok {
		if !coerce {
			...
			if _, ok := t.(*NativeType); !ok { // not native type, refer to time4_native.gno.
				...
				// It does not guarantee convertible, e.g. s1{foo: 1} == 1). (why this is considered as assignable and pass check?).
		...
		if coerce || !coerce && isUntyped(xt) {
			...
			checkAssignable(xt, t, false) // refer to 0_a_2.gno, 0_a_3.gno
		}
		...
		if isUntyped(xt) {
			...
			// main logic for handling nested binary expr
			// e.g. 1 << 2 + 1 << x, will be handled recursively until its type is determined.
	...
	if t != nil && t.Kind() == InterfaceKind {
		if cx.IsUndefined() && cx.T == nil { // if cx is nil, not undefined interface
			if _, ok := t.(*NativeType); !ok { // bypass native nil interface, not support native interface(nil) now
				...
				setConstAttrs(cx)
				debugPP.Printf("after convert, cx: %v, cx.T: %v \n", cx, cx.T)
				return
			}
		}
